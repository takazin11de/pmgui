...
const PmObj pm_widget_obj = {sizeof(PmWidget), 0, 0, 0};
...
int on_radiobox_clicked(PmEvent *event)
{
  PmWidget radiobox = event->widget;
  PmWidget group = event->data;
  PmList list = group->radiobox_list;
  PmListIt it;
  PmWidget *data;
  ...
  for_PmList(list, &it, data)
  {
    if ((*data) == radiobox)
    {
      PmWidget_SetCheck((*data), 1);
    }
    else
    {
      PmWidget_SetCheck((*data), 0);
    }
  }

  PostMessageW(pmgui.hidden_hwnd, PM_WM_RADIO_GROUP_CLICKED, (WPARAM)group, 0);
  return 0;
}
...
PmWidget PmWidget_Init(int widgets_type)
{
  ...
  self->radiobox_list = PmList_New(sizeof(PmWidget), pm_widget_obj);
  ...
  case pmw_radiobox:
    ...
    self->size_y = 30;
    self->hwnd =
        CreateWindowW(pmgui.str_button, self->utf16_text, WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON, self->pos_x,
                      self->pos_y, self->size_x, self->size_y, pmgui.hidden_hwnd, NULL, pmgui.instance, NULL);
    if (self->hwnd == NULL)
    {
    }
...
int PmRadioboxGroup_Add(PmWidget radiobox_group, PmWidget radiobox)
{
  PmList list = radiobox_group->radiobox_list;
  PmListIt it;
  PmWidget *data;
  int f = 0;
  if (radiobox_group->type == pmw_radiobox_group && radiobox->type == pmw_radiobox)
  {
    for_PmList(list, &it, data)
    {
      if ((*data) == radiobox)
    ...
    if (f == 0)
    {
      PmList_PushBack(list, &radiobox);
      PmEvent_Bind(pme_clicked, on_radiobox_clicked, radiobox, radiobox_group);
    }
  }
  return 0;
}
